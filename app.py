  ...
  for tentativa in range(max_tentativas):
    try:
      ...
      if "503" in str(e) and tentativa < max_tentativas - 1:
        ...
        st.warning("Servidor da IA ocupado. Digite o código manualmente abaixo.")
        return None
    ...
    if codigo_final_digitado.strip():
      ...
      if colunas_possiveis:
        ...
        if bobina_encontrada is not None:
          ...
          st.markdown(
              f"""
            <div style="background-color: #064e3b; border: 2px solid #10b981; padding: 20px; border-radius: 12px; color: #ffffff;">
                <h3 style="margin: 0 0 10px 0; color: #34d399 !important; font-size: 18px; font-family: monospace;">STATUS: VALIDADO</h3>
                
                <div style="font-size: 26px; font-weight: bold; background: #022c22; padding: 12px; border-radius: 8px; text-align: center; margin-bottom: 15px; letter-spacing: 2px; font-family: monospace;">
                    {codigo_confirmado}
                </div>
                
                <p style="margin: 5px 0; font-size: 15px;"><b>Manifesto:</b> {navio_selecionado}</p>
                <p style="margin: 5px 0; font-size: 15px;"><b>Item:</b> {item} | <b>BL:</b> {bl}</p>
                <p style="margin: 5px 0; font-size: 15px;"><b>Cliente:</b> {cliente}</p>
                <p style="margin: 5px 0; font-size: 15px;"><b>Destino:</b> {destino}</p>
                <p style="margin: 5px 0; font-size: 15px;"><b>Peso Líq:</b> {peso_liq}t | <b>Peso Brut:</b> {peso_brut}t</p>
            </div>
            """,
              unsafe_allow_html=True,
          )
          ...
          st.markdown(
              """
            <div style="background-color: #7f1d1d; border: 2px solid #ef4444; padding: 20px; border-radius: 12px; color: #ffffff;">
                <h3 style="margin: 0; color: #f87171 !important; font-family: monospace;">STATUS: DIVERGÊNCIA</h3>
                <p style="margin: 5px 0 0 0;">O código informado não foi localizado no manifesto.</p>
            </div>
            """,
              unsafe_allow_html=True,
          )