...
# --- SISTEMA DE AUTENTICAÇÃO E MEMÓRIA DO USUÁRIO ---
if "autenticado" not in st.session_state:
    st.session_state["autenticado"] = False

if "usuario_logado" not in st.session_state:
    st.session_state["usuario_logado"] = "SOIN"
...
if not st.session_state["autenticado"]:
    ...
    with col_auth2:
        with st.container(border=True):
            # Campo de texto livre para digitar o usuário (já puxando "SOIN" como padrão inicial)
            usuario_digitado = st.text_input("Usuário:", value=st.session_state.get("usuario_logado", "SOIN"), key="input_usuario_restrito")
            ...
            if st.button("Autenticar Acesso", type="primary", width='stretch'):
                if senha_digitada == "porto2026":
                    ...
                    # Salva o usuário que foi digitado para persistir nas telas e próximas sessões
                    st.session_state["usuario_logado"] = usuario_digitado.strip().upper() if usuario_digitado else "SOIN"
                    st.rerun()
                else:
...
if st.session_state.modo == "Home":
    st.markdown(f"""
        <div style='text-align: center; padding: 2.5rem 0 1rem 0;'>
            ...
            <p style='color: #94a3b8; font-size: 0.85rem; margin-top: 6px;'>Usuário Ativo: <b style='color: #38bdf8;'>{st.session_state.get('usuario_logado', 'SOIN')}</b></p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with col_Rodape1:
        st.caption("DRX SOLUTIONS - Siderurgia")
    with col_Rodape2:
        if st.button("Bloquear Tela", width='stretch'):
            st.session_state["autenticado"] = False
            st.rerun()
...
# ==========================================
    ...
    if not arquivos_excel:
        ...
        with st.container(border=True):
            ...
            # Usa o usuário salvo em session_state preenchendo automaticamente
            operador_padrao = f"{st.session_state.get('usuario_logado', 'SOIN')} (Porão)"
            operador = st.text_input("Operador Responsável / Porão:", value=operador_padrao)
...
# ==========================================
    ...
    if not arquivos_navios_disp:
        ...
        with st.container(border=True):
            ...
            # Puxa o usuário salvo automaticamente para o armazém também
            operador_padrao_arm = f"{st.session_state.get('usuario_logado', 'SOIN')} (Armazém)"
            op_armazem = st.text_input("Operador Responsável:", value=operador_padrao_arm)
